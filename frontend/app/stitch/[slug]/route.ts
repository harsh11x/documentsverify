...
const STITCH_FILE_BY_SLUG: Record<string, string> = {
  "verify-truth": "verify-truth-monolithic-hero.html",
  "certificate-verification": "certificate-verification.html",
  "register-organization": "register-organization.html",
  "digital-certificate-view": "digital-certificate-view.html",
};

function injectNavigationBindings(html: string, slug: string): string {
  const apiBaseUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000";
  const script = `
<script>
  (function () {
    var pageSlug = ${JSON.stringify(slug)};
...
export async function GET(
  ...
  const fileName = STITCH_FILE_BY_SLUG[params.slug];
  if (!fileName) {
  ...
  try {
    ...
    const hydratedHtml = injectNavigationBindings(html, params.slug);
    ...
    return new NextResponse("Stitch export file missing", { status: 500 });
  }
}
